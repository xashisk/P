 ...
 */
public class PrimitiveDomainVS<T> implements ValueSummary<PrimitiveDomainVS<T>> {
    /**
    ...
    public PrimitiveVS<T> toPrimitiveVS() {
        List<PrimitiveVS<T>> toMerge = new ArrayList<>();
        for (GuardedValue<Domain<T>> guardedValue : this.getGuardedValues()) {
            List<PrimitiveVS<T>> choices = new ArrayList<>();
            for (T value : DomainManager.toConcrete(guardedValue.getValue())) {
                choices.add(new PrimitiveVS<>(value).restrict(guardedValue.getGuard()));
                toMerge.add(NondetUtil.getNondetChoice(choices));
            }
        }
        return (new PrimitiveVS<T>()).merge(toMerge);
    }
